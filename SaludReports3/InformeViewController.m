...
@interface InformeViewController ()
...
-(void)setupArray{
    @try{
        ...
        NSString *stringUrl =  @"http://desarrollo.salud.gob.mx/dgpop/json/muestra_informes.php";
        ...
        for (NSDictionary * item in arrayResults){
            ...
            NSString *nombrePdf;
            id_inf  = [[item objectForKey:@"id_inf"] intValue] ;
            descripcion  = [item objectForKey:@"descripcion"] ;
            nombrePdf  = [item objectForKey:@"nombrePdf"] ;
            Informe *inf1 = [[Informe alloc] initWithId:id_inf andDescripcion:descripcion andNombrePdf:nombrePdf];
            [arrayTemp addObject:inf1];
        }
...
-(IBAction)downloadRepor:(UIButton*)sender{
    ...
    Informe *nor1 = [arrayNormativa objectAtIndex:indexPath.row];
    NSData *pdfData = [[NSData alloc] initWithContentsOfURL:[NSURL URLWithString:[[NSString  stringWithFormat:@"http://desarrollo.salud.gob.mx/dgpop/json/docs/informes/"] stringByAppendingString:nor1.nombrePdf]]];
    ...
    if(success){ //se guardo el PDF????
        NSString *stringForSql =  [NSString stringWithFormat:@"Insert into Informes values(\"%d\", \"%@\", \"%@\")",   nor1.id_in, nor1.descripcion, nor1.nombrePdf];
        NSLog(stringForSql);
...
-(IBAction)deleteRepor:(UIButton*)sender{
    ...
    if ([filemgr removeItemAtPath:
        ...
        NSString *stringForSql =  [NSString stringWithFormat:@"DELETE FROM  Informes  where nombrePdf =  \"%@\"",   nor1.nombrePdf];
        NSLog(stringForSql);
        ...
        if (success) {
            ...
            [mainDelegate.databaseConnection readFromDatabase];
            arrayNormativa = mainDelegate.databaseConnection.arrayInformes;
            [tableView reloadData];
...
(NSIndexPath *)indexPath {
    ...
    if (segmentedControl.selectedSegmentIndex == 0) { //en linea
        NSString *archivoGraficaSeleccionadoCompleto = [[NSString  stringWithFormat:@"http://desarrollo.salud.gob.mx/dgpop/json/docs/informes/"] stringByAppendingString:nor1.nombrePdf];
